...
export function ShopCard({ shop, distance, index = 0 }: ShopCardProps) {
  ...
  const { Icon, color, tint } = getCategoryMeta(shop.category);
  return (
      ...
      <Link to={`/shop/${shop.id}`} className="atlas-card flex h-full flex-col overflow-hidden transition-[box-shadow,border-color] duration-200 hover:border-primary/45 hover:shadow-[0_20px_45px_-26px_hsl(var(--foreground)/0.45)]">
        <div className="relative aspect-[15/10] overflow-hidden bg-muted">
          ...
          <div className="absolute inset-x-3 bottom-3 flex items-center justify-between text-white">
            <span className="atlas-label inline-flex items-center gap-1.5 border border-white/30 bg-black/30 px-2 py-1 backdrop-blur-sm"><Icon className="h-3 w-3" />{shop.category}</span>
            <span className={`atlas-label px-2 py-1 ${shop.isSample ? "bg-white/85 text-[#102a31]" : shop.isOpen ? "bg-primary text-primary-foreground" : "bg-black/60"}`}>{shop.isSample ? "Sample" : shop.isOpen ? "Open" : "Closed"}</span>
          </div>
        </div>
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-auto grid grid-cols-[1fr_auto] gap-x-3 gap-y-3 border-t border-border pt-4">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground" style={{ color: shop.isSample ? color : undefined }}>{shop.priceLevel || "—"}</span>
          </div>
        </div>
      </Link>
    </motion.article>
