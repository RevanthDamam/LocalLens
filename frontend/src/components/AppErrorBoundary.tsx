...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    return <main className="contour-surface grid min-h-screen place-items-center p-6"><section className="max-w-md border border-border bg-card p-7 shadow-elevated"><p className="atlas-label text-primary">Field guide unavailable</p><h1 className="mt-3 font-display text-4xl leading-none tracking-[-0.045em]">We could not load this page.</h1><p className="mt-4 text-sm leading-6 text-muted-foreground">Please refresh the page. If the problem continues, the LocalLens team can use the deployment logs to investigate it.</p><code className="mt-4 block break-words border border-destructive/20 bg-destructive/5 p-3 font-mono text-[11px] leading-5 text-destructive" data-runtime-error>{this.state.error.message}</code><button onClick={() => window.location.reload()} className="mt-6 inline-flex items-center gap-2 bg-secondary px-4 py-3 text-sm font-extrabold text-secondary-foreground transition hover:bg-primary"><RefreshCw className="h-4 w-4" />Refresh LocalLens</button></section></main>;
  }
}
